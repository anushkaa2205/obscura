...
> This is served directly over HTTP for now. HTTPS via CloudFront is built and ready to switch on but currently blocked by an AWS account verification requirement (a support case has been filed) — see [Known limitations](#known-limitations).
...
- **HTTPS is not yet live.** CloudFront distribution creation is blocked by `AccessDenied: Your account must be verified before you can add new CloudFront resources` — a known restriction on newer/lightly-used AWS accounts, resolved by an AWS Support case (filed, pending). The app currently serves over plain HTTP via the ALB directly. For a tool handling photo uploads, this is a real gap between intent and current state, documented rather than hidden.
- **Server-side processing, by necessity not by ideal.** The correct architecture for a pure privacy tool is client-side stripping — nothing ever leaves the browser. Server-side processing was chosen here to demonstrate cloud architecture (S3, IAM, compute). A production version would strip metadata in-browser via JavaScript and use the server only for optional storage, eliminating server-side trust entirely.
